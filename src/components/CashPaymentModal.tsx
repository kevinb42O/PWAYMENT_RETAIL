...
export const CashPaymentModal: React.FC<Props> = ({ open, onClose, totalCents, onConfirm }) => {
  const [tenderedCents, setTenderedCents] = useState<number>(totalCents);

  useEffect(() => {
    if (open) setTenderedCents(totalCents);
  }, [open, totalCents]);
  ...
  const press = (digit: string) => {
    setTenderedCents((cur) => {
      ...
      const next = cur * 10 + Number(digit);
      if (next > 99_999_900) return cur;
      return next;
    });
  };

  const back = () => setTenderedCents((cur) => Math.floor(cur / 10));
  const clear = () => setTenderedCents(0);
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-3">
            <div className="text-zinc-500 text-xs uppercase tracking-wider">Totaal</div>
            <div className="text-2xl font-bold tabular-nums">{formatEUR(totalCents)}</div>
          </div>
          <div
          ...
          >
            ...
            <div
              className={`text-2xl font-bold tabular-nums ${
                insufficient ? 'text-red-300' : change === 0 ? 'text-emerald-300' : 'text-amber-300'
              }`}
            >
        ...
        <div>
          <div className="text-xs uppercase tracking-wider text-zinc-500 mb-2">Snelkeuze</div>
        ...
        <div>
          <div className="grid grid-cols-3 gap-2">
            ...
            <button onClick={clear} className="py-4 rounded-lg bg-zinc-900 hover:bg-zinc-800 font-bold text-zinc-400">
              C
            </button>
            <button onClick={() => press('0')} className="py-4 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-2xl font-bold">
              0
            </button>
            <button onClick={back} className="py-4 rounded-lg bg-zinc-900 hover:bg-zinc-800 font-bold text-zinc-400">
              ⌫
            </button>
          </div>
          <p className="text-[11px] text-zinc-500 mt-2">
            Tip: cijfers schuiven van rechts naar links — typ "2000" voor €20,00.
          </p>
        </div>
      </div>
    </Modal>
