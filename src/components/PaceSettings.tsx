...
import { PaceMark, type PacePerformance, type PacePose } from "../pace/PaceMark";
...
export const PaceSettings = ({ subTab = "overview" }: PaceSettingsProps) => {
  ...
  const [posePreview, setPosePreview] = useState<PacePose | null>(null);
  ...
  useEffect(() => {
    if (!preview && !posePreview) return;
    const timer = window.setTimeout(() => { setPreview(null); setPosePreview(null); }, 3_100);
  ...
  const playPreview = (performance: PacePerformance) => { setPosePreview(null); setPreview(performance); };
  const playPosePreview = (pose: PacePose) => { setPreview(null); setPosePreview(pose); };
