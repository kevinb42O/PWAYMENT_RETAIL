      ...
      <div className="space-y-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 bg-slate-50 py-2 pl-10 pr-4 text-slate-900 placeholder-slate-500 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
            autoFocus
          />
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map(c => (
              <button
                key={c.id}
                onClick={() => onLink(c.id)}
                className="group w-full rounded-lg border border-slate-300 bg-white p-3 text-left transition-colors hover:border-slate-400 hover:bg-slate-100 flex items-center justify-between"
              >
