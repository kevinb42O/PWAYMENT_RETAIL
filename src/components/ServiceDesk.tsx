...
export const ServiceDesk: React.FC = () => {
  ...
  return (
    <main className="flex-1 overflow-hidden bg-slate-50 print:overflow-visible">
      <div className="flex h-full flex-col print:hidden">
        <section className="border-b border-slate-200 bg-white px-4 py-5 sm:px-6 lg:px-8">
          <div className="mx-auto flex max-w-[1600px] flex-col gap-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-sky-600"><Wrench size={15} /> Service operations</div>
              ...
              <button type="button" onClick={() => { setDraft(EMPTY_DRAFT); setAttachments([]); setIntakeOpen(true); }} className="inline-flex items-center gap-2 rounded-xl bg-sky-600 px-4 py-2.5 text-sm font-extrabold text-white shadow-sm hover:bg-sky-700"><Plus size={17} /> Nieuwe intake</button>
            </div>
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              {([
                [counters.open, "Nieuw ontvangen", ClipboardCheck, "text-sky-700 bg-sky-50"],
                [counters.active, "In behandeling", Wrench, "text-violet-700 bg-violet-50"],
                [counters.ready, "Klaar voor afhaling", PackageCheck, "text-emerald-700 bg-emerald-50"],
                [formatEUR(counters.unpaid), "Openstaand", Banknote, "text-amber-700 bg-amber-50"],
              ] as Array<[string | number, string, React.ElementType, string]>).map(([value, label, Icon, tone]) => {
                const MetricIcon = Icon as React.ElementType;
                return <div key={String(label)} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3.5"><span className={`rounded-xl p-2 ${tone}`}><MetricIcon size={18} /></span><div><div className="text-lg font-black text-slate-950">{value}</div><div className="text-[11px] font-semibold text-slate-500">{label}</div></div></div>;
              })}
            </div>
          </div>
        </section>
        ...
        <div className="mx-auto grid min-h-0 w-full max-w-[1600px] flex-1 gap-0 px-4 pb-4 pt-4 sm:px-6 lg:grid-cols-[390px_minmax(0,1fr)] lg:px-8">
          <section className="flex min-h-0 flex-col overflow-hidden rounded-t-3xl border border-slate-200 bg-white lg:rounded-l-3xl lg:rounded-tr-none">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-2">
              {filteredOrders.length === 0 ? <div className="m-2 rounded-2xl bg-slate-50 p-6 text-center"><Wrench size={28} className="mx-auto text-slate-300" /><div className="mt-3 text-sm font-extrabold text-slate-700">Geen dossiers gevonden</div><div className="mt-1 text-xs leading-5 text-slate-500">Maak een intake aan of wijzig de filter.</div></div> : filteredOrders.map((order) => (
                <button key={order.id} type="button" onClick={() => setSelectedId(order.id)} className={`mb-2 w-full rounded-2xl border p-3.5 text-left transition ${selected?.id === order.id ? "border-sky-300 bg-sky-50 shadow-sm" : "border-transparent hover:border-slate-200 hover:bg-slate-50"}`}>
          ...
          <section className="min-h-0 overflow-y-auto rounded-b-3xl border border-t-0 border-slate-200 bg-white lg:rounded-r-3xl lg:rounded-bl-none lg:border-l-0 lg:border-t">
            {!selected ? (
              <div className="flex min-h-96 flex-col items-center justify-center p-10 text-center"><span className="rounded-3xl bg-slate-100 p-5 text-slate-400"><ClipboardCheck size={38} /></span><h2 className="mt-5 text-lg font-black text-slate-800">Uw eerste dossier begint bij de intake</h2><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">Leg klant, toestel, staat, route, prijs en foto’s één keer vast. Vanaf dan volgt iedereen exact dezelfde waarheid.</p><button type="button" onClick={() => setIntakeOpen(true)} className="mt-5 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white">Nieuwe intake</button></div>
            ) : (
                ...
                <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px]">
                  <div className="space-y-6">
                    ...
                    <section className="rounded-2xl border border-slate-200 p-5">
                      ...
                      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-950 p-4 text-white"><div><div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Totaal / openstaand</div><div className="mt-1 text-xl font-black">{formatEUR(selected.totalCents)} <span className="text-sm font-semibold text-slate-400">/ {formatEUR(Math.max(0, selected.totalCents - selected.paidCents))}</span></div></div><div className="flex gap-2"><button type="button" onClick={() => void saveDetails()} className="rounded-xl bg-white/10 px-3 py-2 text-xs font-bold hover:bg-white/20">Prijzen bewaren</button><button type="button" onClick={() => void addBalanceToCart()} className="inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-3 py-2 text-xs font-black text-emerald-950 hover:bg-emerald-400"><ShoppingCart size={15} /> Naar kassa</button></div></div>
                    </section>
                  ...
                  <aside className="space-y-5">
                    <section className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center gap-2"><UserRound size={17} className="text-sky-600" /><h3 className="text-sm font-black text-slate-900">Klant</h3></div><div className="mt-3 text-sm font-extrabold text-slate-900">{selected.customerName}</div>{selected.customerEmail && <div className="mt-1 break-all text-xs text-slate-500">{selected.customerEmail}</div>}{selected.customerPhone && <div className="mt-1 text-xs text-slate-500">{selected.customerPhone}</div>}<div className="mt-4 grid gap-2"><button type="button" disabled={!selected.customerEmail} onClick={openCustomerEmail} className="inline-flex items-center justify-center gap-2 rounded-xl bg-sky-600 px-3 py-2.5 text-xs font-extrabold text-white hover:bg-sky-700 disabled:opacity-40"><Mail size={15} /> {selected.status === "ready" ? "Mail: klaar" : "Mail bevestiging"}</button><button type="button" disabled={!selected.customerPhone} onClick={callCustomer} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-extrabold text-slate-700 hover:bg-slate-50 disabled:opacity-40"><Phone size={15} /> Bel klant</button></div></section>
                    ...
                    <section className="rounded-2xl bg-slate-950 p-4 text-white"><div className="flex items-center gap-2 text-xs font-black"><Clock3 size={15} className="text-sky-400" /> Belofte</div><div className="mt-2 text-sm font-extrabold">{formatDate(selected.promisedAt)}</div><div className="mt-1 text-[11px] text-slate-400">Aangemaakt {formatDateTime(selected.createdAt)}</div></section>
                  </aside>
                </div>
              </div>
            )}
      ...
      <Modal open={intakeOpen} onClose={() => setIntakeOpen(false)} title="Nieuwe herstelintake" subtitle="Leg één keer alles vast wat winkel, technieker en klant nodig hebben." icon={<Wrench size={19} />} size="5xl" footer={<div className="flex w-full items-center justify-between gap-3"><div className="text-xs text-slate-500"><strong>{attachments.length}</strong> foto{attachments.length === 1 ? "" : "’s"} · route: <strong>{routeLabel(draft.route)}</strong></div><div className="flex gap-2"><button type="button" onClick={() => setIntakeOpen(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-700">Annuleren</button><button type="button" disabled={isSaving} onClick={() => void createOrder()} className="inline-flex items-center gap-2 rounded-xl bg-sky-600 px-5 py-2.5 text-sm font-extrabold text-white hover:bg-sky-700 disabled:opacity-50">{isSaving ? <RefreshCw size={16} className="animate-spin" /> : <CheckCircle2 size={16} />} Dossier aanmaken</button></div></div>}>
