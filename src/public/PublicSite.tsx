...
const faqs = [
  ...
  ['Wat gebeurt er na de proefperiode?', 'PWAYMENT Basis blijft gratis. Je kunt Retail Professional 14 dagen gratis proberen. Daarna kies je of je Professional activeert; anders blijf je Basis gebruiken.'],
];
    ...
    <section className="pw-hero pw-shell">
      <motion.div className="pw-hero-copy" initial="hidden" animate="visible" variants={stagger}>
        ...
        <motion.div className="pw-hero-actions" variants={fadeUp}>
          <a href="/register?plan=professional" className="pw-button pw-button-dark">Probeer Professional 14 dagen gratis <ArrowRight size={17} /></a>
          <a href="/demo" className="pw-text-link">Plan een persoonlijke demo <ArrowRight size={15} /></a>
        </motion.div>
        <motion.div className="pw-hero-note" variants={fadeUp}><CircleCheck size={16} /> Basis blijft gratis · Professional 14 dagen gratis proberen</motion.div>
      </motion.div>
...
const PricingPage = () => {
  ...
  return <>
    <AnimatedPageHero className="pw-pricing-hero" eyebrow="Prijzen" title={<>Alles wat je nodig hebt.<br />Precies op het juiste moment.</>} intro="PWAYMENT Basis blijft gratis. Probeer Retail Professional 14 dagen gratis en groei zonder van systeem te veranderen. Alle softwareprijzen zijn exclusief btw." actions={<div className="pw-cycle" role="group" aria-label="Facturatieperiode"><button className={cycle === 'monthly' ? 'active' : ''} onClick={() => setCycle('monthly')}>Maandelijks</button><button className={cycle === 'yearly' ? 'active' : ''} onClick={() => setCycle('yearly')}>Jaarlijks <span>Bespaar</span></button></div>} />
