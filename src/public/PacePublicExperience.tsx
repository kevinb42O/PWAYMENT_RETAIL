  ...
  return (
    <section className={`pw-pace-story${compact ? " is-compact" : ""}`} aria-label="Interactieve demonstratie van Pace">
      ...
      <div className="pw-pace-demo">
        <div className="pw-pace-tabs" role="tablist" aria-label="Pace-situaties">{(Object.keys(SCENES) as SceneId[]).map((id) => <button key={id} type="button" role="tab" aria-selected={scene === id} onClick={() => setScene(id)}>{SCENES[id].label}</button>)}</div>
        <div className="pw-pace-console"><div className="pw-pace-console-top"><span>PWAYMENT · LIVE CONTEXT</span><b><i /> PACE</b></div><AnimatePresence mode="wait" initial={false}><motion.div className="pw-pace-console-body" key={scene} initial={reducedMotion ? false : { opacity: 0, y: 10, scale: .99 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={reducedMotion ? undefined : { opacity: 0, y: -7 }} transition={{ duration: .28, ease: [0.22, 1, 0.36, 1] }}><div className="pw-pace-character"><PaceMark size={86} active emotion={current.emotion} tone={current.tone} motionMode={reducedMotion ? "off" : "full"} /></div><div className="pw-pace-message"><span>{scene === "offline" ? <CloudOff size={13} /> : scene === "rush" ? <EyeOff size={13} /> : <Sparkles size={13} />} {current.eyebrow}</span><h3>{current.title}</h3><p>{current.body}</p><div><Check size={14} /> {current.note}</div></div></motion.div></AnimatePresence></div>
      </div>
    </section>
  );
};
