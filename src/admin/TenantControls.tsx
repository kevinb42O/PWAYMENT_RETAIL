...
export const TenantControls = ({ storeId, detail, onDeleted, section }: {
  ...
  const submitDelete = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage(result.archived
        ? `${result.deleted_store_name} is buiten gebruik gesteld. Financiële bewijsdata blijft vergrendeld bewaard tot minstens ${dateTime(result.retention_until)}.`
        : `${result.deleted_store_name} is verwijderd omdat er geen financiële bewijsdata bestond. ${result.deleted_orphan_users} losstaande gebruikersaccount(s) zijn eveneens verwijderd.`);
      onDeleted();
  ...
  return <section className="space-y-5">
    ...
    {showDanger && <form onSubmit={submitDelete} className="rounded-2xl border border-rose-200 bg-rose-50 p-5"><div className="flex gap-3"><ShieldAlert className="mt-0.5 text-rose-700" size={21} /><div><h3 className="text-sm font-extrabold text-rose-950">Tenant gecontroleerd buiten gebruik stellen</h3><p className="mt-1 max-w-3xl text-xs leading-5 text-rose-900">Toegang en abonnement worden beëindigd. Als financiële bewijsdata bestaat, blijft die vergrendeld bewaard tot het einde van de wettelijke retentie; alleen een lege tenant zonder financiële bewijsdata wordt fysiek verwijderd.</p></div></div><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-rose-950">Typ exact: {detail.store.name}<input required value={deleteName} onChange={(event) => setDeleteName(event.target.value)} className="mt-1.5 w-full rounded-xl border border-rose-200 bg-white px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-rose-950">Reden voor offboarding<textarea required minLength={8} value={deleteReason} onChange={(event) => setDeleteReason(event.target.value)} rows={2} className="mt-1.5 w-full resize-none rounded-xl border border-rose-200 bg-white px-3 py-2.5 text-sm" /></label></div><Button type="submit" variant="danger" className="mt-4" disabled={deleting || deleteName !== detail.store.name}>{deleting ? "Buiten gebruik stellen…" : <><Trash2 size={15} /> Beëindig tenant veilig</>}</Button></form>}
  </section>;
};
