...
export class POSDatabase extends Dexie {
  ...
  shifts!: Table<Record<string, unknown>, number>;
  ...
  constructor() {
      ...
      .upgrade(async (tx) => {
        ...
        await tx
          ...
          .modify((row: Record<string, unknown>) => {
            ...
            const num = (k: string) => Math.round(((r[k] as number) ?? 0) * 100);
            if (r.totalRevenueCents == null && typeof r.totalRevenue === 'number') {
    ...
    this.version(6)
      ...
      .upgrade(async (tx) => {
        ...
        const uniqueNames = Array.from(new Set(products.map((p) => p.category).filter(Boolean)));
