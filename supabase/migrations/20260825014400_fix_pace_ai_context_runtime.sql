...
-- Runtime fix: COALESCE is SQL syntax and must not be schema-qualified. The caller's auth.uid()
...
begin
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'operations', pg_catalog.jsonb_build_object(
      'webshopOrdersByStatus', coalesce((select pg_catalog.jsonb_object_agg(grouped.status, grouped.total) from (select webshop.status, pg_catalog.count(*) total from public.webshop_orders webshop where webshop.store_id = target_store_id and not webshop.is_demo group by webshop.status) grouped), '{}'::jsonb),
