...
-- A bounded, read-only context projection for Pace AI. The caller's auth.uid()
...
declare
  ...
  store_timezone text;
  store_currency text;
  normalized_query text := pg_catalog.lower(pg_catalog.left(pg_catalog.coalesce(user_query, ''), 240));
  local_today date;
  result jsonb;
begin
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'subscription', pg_catalog.coalesce((
      select pg_catalog.jsonb_build_object(
    ...
    'catalog', pg_catalog.jsonb_build_object(
      ...
      'matchingProducts', pg_catalog.coalesce((
      ...
      'lowestStock', pg_catalog.coalesce((
    ...
    'sales', pg_catalog.jsonb_build_object(
      'today', pg_catalog.coalesce((
        select pg_catalog.jsonb_build_object(
          'transactionCount', pg_catalog.count(*) filter (where txn.kind = 'sale'),
          'refundCount', pg_catalog.count(*) filter (where txn.kind = 'refund'),
          'netTotalCents', pg_catalog.coalesce(pg_catalog.sum(case when txn.kind = 'refund' then -pg_catalog.abs(txn.total_cents) else txn.total_cents end), 0)
        )
      ...
      'last30Days', pg_catalog.coalesce((
        select pg_catalog.jsonb_build_object(
          'transactionCount', pg_catalog.count(*) filter (where txn.kind = 'sale'),
          'refundCount', pg_catalog.count(*) filter (where txn.kind = 'refund'),
          'netTotalCents', pg_catalog.coalesce(pg_catalog.sum(case when txn.kind = 'refund' then -pg_catalog.abs(txn.total_cents) else txn.total_cents end), 0),
          'discountCents', pg_catalog.coalesce(pg_catalog.sum(txn.discount_cents), 0)
        )
        from public.transactions txn
        where txn.store_id = target_store_id
          and not txn.is_demo
      ...
      'paymentMix30Days', pg_catalog.coalesce((
      ...
      'recentTransactions', pg_catalog.coalesce((
        ...
        from (
          ...
          from public.transactions txn
          where txn.store_id = target_store_id and not txn.is_demo
          order by txn.occurred_at desc
          limit 12
        ) recent
      ), '[]'::jsonb),
      'matchingTransactions', pg_catalog.coalesce((
    ...
    'customers', pg_catalog.jsonb_build_object(
      ...
      'matchingCustomers', pg_catalog.coalesce((
    ...
    'operations', pg_catalog.jsonb_build_object(
      'webshopOrdersByStatus', pg_catalog.coalesce((select pg_catalog.jsonb_object_agg(grouped.status, grouped.total) from (select webshop.status, pg_catalog.count(*) total from public.webshop_orders webshop where webshop.store_id = target_store_id and not webshop.is_demo group by webshop.status) grouped), '{}'::jsonb),
      'serviceOrdersByStatus', pg_catalog.coalesce((select pg_catalog.jsonb_object_agg(grouped.status, grouped.total) from (select service.status, pg_catalog.count(*) total from public.service_orders service where service.store_id = target_store_id group by service.status) grouped), '{}'::jsonb),
      'purchaseOrdersByStatus', pg_catalog.coalesce((select pg_catalog.jsonb_object_agg(grouped.status, grouped.total) from (select purchase.status, pg_catalog.count(*) total from public.purchase_orders purchase where purchase.store_id = target_store_id and not purchase.is_demo group by purchase.status) grouped), '{}'::jsonb),
      ...
      'latestDailyReport', pg_catalog.coalesce((
        select pg_catalog.jsonb_build_object(
