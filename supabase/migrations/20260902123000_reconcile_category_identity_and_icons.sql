...
-- A category created by an older local client can have a different external
-- ID than its already-existing server category. The name + direct parent is
-- the stable category identity, so reuse that canonical row instead of
-- rejecting a harmless setting change as a duplicate.
...
begin
  ...
  -- If the external ID came from a former device, adopt the existing server
  -- row with the same visible path. This is intentionally limited to an exact
  -- sibling match, so a genuine new category stays a new category.
